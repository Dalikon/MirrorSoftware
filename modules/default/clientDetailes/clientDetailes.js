class clientDetailes extends Module {
	...
	async showPopup(targetClient) {
		...
		cursorToggle.addEventListener("click", () => {
			...
			trackerSocket.sendNotification("TOGGLE_CURSOR_X", {
				client: this.clientConfig.name,
	...
	moduleSettingsElement(moduleSettings = document.getElementById("moduleSettings")) {
		...
		for (const [index, moduleConfig] of modules.entries()) {
			moduleSettings.appendChild(
				this.moduleControlElement(this.clientConfig.name, moduleConfig, index),
			);
		}
	}
	...
	moduleControlElement(clientName, moduleConfig, index) {
		...
		hideShowBtn.addEventListener("click", () => {
			if (hideShowBtn.textContent === "Hide") {
				trackerSocket.sendNotification("HIDE_MODULE_X", {
				...
				trackerSocket.sendNotification("SHOW_MODULE_X", {
					module: moduleConfig.module,
					id: moduleID,
					client: clientName,
				});
		...
		susResBtn.addEventListener("click", () => {
			if (susResBtn.textContent === "Suspend") {
				trackerSocket.sendNotification("SUSPEND_MODULE_X", {
					module: moduleConfig.module,
					id: moduleID,
					client: clientName,
				});
				...
				trackerSocket.sendNotification("RESUME_MODULE_X", {
					module: moduleConfig.module,
					id: moduleID,
					client: clientName,
				});
	...
	async userButtonsElement(tClient) {
		...
		container.addEventListener("click", (event) => {
			...
			trackerSocket.sendNotification("CHANGE_USER_X", {
				client: this.clientConfig.name,
