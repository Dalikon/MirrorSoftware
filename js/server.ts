...
import type { ModuleSocketPayload, UserSocketPayload } from "../types/socket.js";
...
class Server {
    ...
    userServiceEndpoints(): void {
        this.app.post("/get-user/:userName", (req, res) => {
            ...
            req.on("end", () => {
                let filePath = path.join(path.resolve(__dirname, "../.."), "/configs", "/" + clientName, "/users", "/" + fileName);
                if (!fs.existsSync(filePath)) {
                    filePath = path.join(path.resolve(__dirname, "../.."), "/configs", "/users", "/" + fileName);
                    if (!fs.existsSync(filePath)) {
    ...
    loadTrackerFile(): void {
        try {
            ...
            this.trackedClients = tracked.map((obj) => ClientTracker.fromObject(obj as Parameters<typeof ClientTracker.fromObject>[0]));
