...
export async function fetchConfig(name?: string): Promise<unknown> {
	...
	const jsonUrl = name ? `/${clientName}/users/${name}.json` : `/${clientName}/${clientName}.json`;
...
export async function fetchClientConfig(clientName: string): Promise<unknown> {
	...
	if (!response.ok)
		throw new Error(`HTTP error fetching client config for ${clientName}: ${response.status}`);
	return response.json();
}
...
export async function fetchUserConfig(clientName: string, userName: string): Promise<unknown> {
	...
	if (!response.ok)
		throw new Error(
			`HTTP error fetching user config ${userName}@${clientName}: ${response.status}`,
		);
	return response.json();
}
