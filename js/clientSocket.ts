...
export class ClientSocket {
	...
	constructor(moduleName: string, query: Record<string, string> | null = null) {
		...
		const onevent = (this.socket as unknown as Record<string, unknown>)["onevent"] as (packet: {
			data?: unknown[];
		}) => void;

		(this.socket as unknown as Record<string, unknown>)["onevent"] = (packet: {
			data?: unknown[];
		}) => {
			if (packet?.data) {
		...
		(this.socket as unknown as { on(ev: string, cb: (...a: unknown[]) => void): void }).on(
			"*",
			(...args: unknown[]) => {
